...
sys.path.append("envs")
from tiny_alchemy import envs as alchemy_envs
...
def make_train(config):
    ...
    def train(rng):
        ...
        def init_agent(rng, agent_id):
            ...
            _action = basic_env.action_space(params=env_params).sample(rng)
        ...
        def _update_step(runner_state, unused):
            ...
            shared_exp = jax.tree_map(lambda x: jnp.squeeze(x,axis=-1), shared_exp)
            #timestep = jax.tree_map(lambda x: jnp.expand_dims(x,axis=-1), timestep)
